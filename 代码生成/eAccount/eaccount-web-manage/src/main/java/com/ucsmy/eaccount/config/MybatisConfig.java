...
import com.ucsmy.eaccount.config.interceptor.GlobalTimeUpdateIntercept;
...
public class MybatisConfig {
    ...
    public SqlSessionFactory sqlSessionFactoryBean(DataSource dataSource, MybatisProperties properties) throws Exception {
        ...
        //自动生成创建时间拦截器
        GlobalTimeUpdateIntercept insertInterceptor = new GlobalTimeUpdateIntercept();

        // 流水号异常处理拦截器
        MybatisSerialNumberExceptionInterceptor serialNumberInterceptor = new MybatisSerialNumberExceptionInterceptor();
        bean.setPlugins(new Interceptor[] { interceptor, logInterceptor, serialNumberInterceptor,insertInterceptor });
